...
{
    public class UInt32SyncPacket : AbstractObjectSyncPacket<uint>
    {
        public UInt32SyncPacket(
            ushort objectSyncId,
            string propertyName,
            SyncOptions syncOptions,
            uint newValue) : base(objectSyncId, propertyName, syncOptions, newValue)
        {
        }
        ...
        public override ReadOnlyMemory<byte> Serialize()
        {
            ...
            var totalLength = sizeof(ushort) + sizeof(uint) + propertyNameMemory.Length;
        ...
        public static UInt32SyncPacket Deserialize(ReadOnlyMemory<byte> memory)
        {
            var index = 0;
            var objectSyncId = BitConverter.ToUInt16(memory.Span.SliceAndIncrement(ref index, sizeof(ushort)));
            var newValue = BitConverter.ToUInt32(memory.Span.SliceAndIncrement(ref index, sizeof(uint)));
            var propertyName = Encoding.UTF8.GetString(memory.Span[index..]);

            return new UInt32SyncPacket(objectSyncId, propertyName, SyncOptions.None, newValue);
        }
    }
}